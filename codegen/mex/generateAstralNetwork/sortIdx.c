...
void sortIdx(const emlrtStack *sp, emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  i = idx->size[0];
  idx->size[0] = nBlocks;
  emxEnsureCapacity_int32_T(sp, idx, i, &oc_emlrtRTEI);
  idx_data = idx->data;
  i = (nBlocks < 3200);
  if (i) {
...
#pragma omp parallel for num_threads(sortIdx_numThreads)
  ...
  st.site = &pc_emlrtRSI;
  emxInit_int32_T(&st, &iwork, &pc_emlrtRTEI);
  bLen = iwork->size[0];
  iwork->size[0] = nBlocks;
  emxEnsureCapacity_int32_T(&st, iwork, bLen, &pc_emlrtRTEI);
  iwork_data = iwork->data;
  if (i) {
    for (i1 = 0; i1 < nBlocks; i1++) {
      iwork_data[i1] = 0;
...
#pragma omp parallel for num_threads(sortIdx_numThreads)
  ...
  emxInit_real_T(&st, &xwork, 1, &rc_emlrtRTEI);
  i = xwork->size[0];
  xwork->size[0] = nBlocks;
  emxEnsureCapacity_real_T(&st, xwork, i, &qc_emlrtRTEI);
  xwork_data = xwork->data;
